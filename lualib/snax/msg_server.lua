...
local ipsum = {}
local daynum = tonumber(os.date("%j"))
...
function server.getipsum()
	return table.size(ipsum)
end
...
function server.start(conf)
	...
	local CMD = {
		...
		get_uids = assert(conf.get_uids),
		kick_outs = assert(conf.kick_outs),
		kick_out = assert(conf.kick_out),
		get_uids_names = assert(conf.get_uids_names),
		kick_addiction = assert(conf.kick_addiction),
		get_ipsum = assert(conf.get_ipsum)
	}
	...
	---设置IP访问次数
	local function setipsum(addr)
		local time = tonumber(os.date("%j"))
		local address = string.match(addr, "(.*):(.*)")
		if time == daynum then
			ipsum[address] = (ipsum[address] or 0) + 1
		else
			ipsum[address] = 1
			daynum = time
		end
	...
	local function auth(fd, addr, msg, sz)
		...
		if not ok then
			skynet.error(result)
			skynet.error("111111123333333331111, result:", result)
	...
	-- 临时队列， 后期扩展断线重连功能时要考虑怎么处理
	function handler.message(fd, msg, sz)
		local addr = handshake[fd]
		if addr and authed[fd] == nil then
			authed[fd] = fd
			auth(fd,addr,msg,sz)
			handshake[fd] = nil
		else
